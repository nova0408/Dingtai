...
from .types import BallObservation, BallPoseDetectionConfig, BallPoseFrame
...
class BallPoseDetector:
    ...
    def __init__(self, config: BallPoseDetectionConfig | None = None) -> None:
        self._config = config if config is not None else BallPoseDetectionConfig()

    def detect(self, frame: BallPoseFrame, priors: list[BallPosePrior]) -> list[BallObservation]:
        masks = self._build_color_masks(frame.color_bgr)
        ranked: dict[str, list[_BallDetection]] = {}
        for prior in priors:
            mask = masks.get(prior.color_hex)
            if mask is None:
                ranked[prior.color_hex] = [self._missing_detection(prior, "missing_color_mask")]
                continue
            candidates = self._collect_color_candidates(prior.color_hex, mask)
        ...
        return [self._to_observation(detection, priors) for detection in detections]

    def estimate_pose(self, detections: list[BallObservation], priors: list[BallPosePrior]) -> _PoseFitResult | None:
        model_points: list[np.ndarray] = []
        camera_points: list[np.ndarray] = []
        for prior in priors:
            match = next((item for item in detections if item.name == prior.name and item.center_mm is not None), None)
            if match is None:
                continue
        ...
        if len(model_points) < 3:
            return None
        return self._fit_rigid_transform(np.stack(model_points, axis=0), np.stack(camera_points, axis=0))
    ...
    def _rank_ball_candidates(
        ...
        if not detections:
            return [self._missing_detection(prior, "no_depth_candidate")]
        detections.sort(key=lambda item: item.score, reverse=True)
        return detections

    def _estimate_center_mm(self, frame: BallPoseFrame, mask: np.ndarray) -> tuple[np.ndarray | None, int]:
    ...
    def _resolve_duplicates(
        ...
        for prior in priors:
            ...
            resolved[prior.name] = ranked[0] if ranked else self._missing_detection(prior, "missing")
    ...
    @staticmethod
    def _estimate_physical_radius_mm(center_mm: np.ndarray, radius_px: float, intrinsics: BallPoseFrame) -> float:
        focal = 0.5 * (float(intrinsics.fx) + float(intrinsics.fy))
        return float(radius_px) * float(center_mm[2]) / max(1e-6, focal)

    @staticmethod
    def _estimate_projected_radius_px(center_mm: np.ndarray, ball_radius_mm: float, intrinsics: BallPoseFrame) -> float:
        focal = 0.5 * (float(intrinsics.fx) + float(intrinsics.fy))
        return float(ball_radius_mm) * focal / max(1e-6, float(center_mm[2]))

    @staticmethod
    def _score_inside_image(center_px: tuple[float, float], radius_px: float, frame: BallPoseFrame) -> float:
