...
def _build_priors_from_capture(captured: dict[str, Any]) -> list[BallPosePriorInfo]:
    ...
    ordered = [lookup.get(color_hex) for color_hex in ordered_colors]
    ...
    for item, color_hex in zip(ordered, ordered_colors, strict=False):
...
def _build_local_pose_overlay(
    ...
    if response.debug is None or response.debug.detection_overlay_bgr is None:
        return None
    overlay = np.asarray(response.debug.detection_overlay_bgr, dtype=np.uint8).copy()
