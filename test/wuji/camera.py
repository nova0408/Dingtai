...
DEFAULT_CAMERA_NAME = "all"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "test" / "wuji" / "artifacts" / "camera_probe"
DEFAULT_HOST = load_wuji_robot_network_config().base_control_ip
DEFAULT_CAPTURE_RGB = True
DEFAULT_CAPTURE_RGBD = True
...
def _probe_one_camera(
    ...
    if capture_rgbd and status.online:
        try:
            ...
            artifacts.depth_u16_path = str(_save_rgb_frame(output_dir, frame, "rgbd_color"))
            ...
            rgbd_ok = True
            rgbd_summary = (
                f"RGBD 首帧保存成功: color={artifacts.depth_u16_path}, "
...
def _save_depth_frame(output_dir: Path, frame: WujiCameraFrame) -> tuple[Path, Path]:
    ...
    if np.any(valid_mask):
        ...
        if max_value > min_value:
            gray[valid_mask] = np.clip((valid_values - min_value) * 255.0 / (max_value - min_value), 0.0, 255.0).astype(np.uint8)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        camera_arg, output_arg, host_arg, rgb_arg, rgbd_arg, request_timeout_arg, stream_timeout_arg = _parse_cli(sys.argv[1:])
        main(
