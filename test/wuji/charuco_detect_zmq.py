...
from src.wuji import SUPPORTED_WUJI_ZMQ_CAMERAS_LOCAL, WujiCameraFrame, WujiCameraName, WujiZmqCameraClient
...
DEFAULT_CAMERA_NAME: WujiCameraName = "head_camera"  # 头部 ZMQ 相机名
DEFAULT_ZMQ_HOST = WUYOU_HOST  # ZMQ 相机服务主机地址
DEFAULT_CONTROL_PORT = 5570  # ZMQ 控制口远端端口，单位 端口号
DEFAULT_HEAD_STREAM_PORT = 5560  # 头部相机数据口远端端口，单位 端口号
...
DEFAULT_HEAD_SETTLE_S = 1.0  # 头部到位等待时间，单位 s
...
def main(config: AppConfig) -> None:
    ...
    client: WujiZmqCameraClient | None = None
    control_tunnel_process = None
    stream_tunnel_process = None
    head_tunnel_process = None
    ...
    try:
        ...
        stream_tunnel_process = start_ssh_tunnel(
            DEFAULT_HEAD_STREAM_PORT,
            remote_host=DEFAULT_ZMQ_HOST,
            ssh_alias=WUYOU_SSH_ALIAS,
        )
        time.sleep(DEFAULT_FORWARD_WAIT_S)

        head_tunnel_process, head_channel = create_wuyou_channel(DEFAULT_PORT)
        ...
        status_response = client.get_camera_status(DEFAULT_CAMERA_NAME)
        if not status_response.online:
            raise RuntimeError("头部 ZMQ 相机离线，无法继续 ChArUco 检测")
        if not status_response.color_enabled:
            status_response = client.set_camera_color_enabled(DEFAULT_CAMERA_NAME, True)

        intrinsics_response = client.get_camera_intrinsics(DEFAULT_CAMERA_NAME)
        calibration = _read_camera_calibration(intrinsics_response)
        logger.info(
            "Charuco eval Wuji head ZMQ stream target: "
            f"camera={DEFAULT_CAMERA_NAME}, calibration={calibration.width}x{calibration.height}, "
            f"online={status_response.online}, color_enabled={status_response.color_enabled}, "
            f"depth_enabled={status_response.depth_enabled}"
        )
        cv2.namedWindow(DEFAULT_WINDOW_NAME, cv2.WINDOW_NORMAL)
        frame_index = 0
        latest_preview: np.ndarray | None = None

        for frame in client.stream_camera_rgb_frames(DEFAULT_CAMERA_NAME):
            color_bgr = _ensure_bgr_uint8(frame)
            if color_bgr.size == 0:
                logger.warning("头部 ZMQ 相机返回空图像，跳过本帧")
                continue
        ...
        if head_tunnel_process is not None:
            stop_ssh_process(head_tunnel_process)
...
def _set_head_fixed_pose(head: WujiHeadClient) -> None:
    logger.info(
        "设置头部固定姿态 yaw {:.1f} deg，pitch {:.1f} deg",
    ...
    logger.success("头部状态 yaw {:.1f} deg，pitch {:.1f} deg", yaw_deg, pitch_deg)
