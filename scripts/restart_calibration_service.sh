...
SERVICE_UNIT="calibration.service"
SERVICE_PORT="6600"
...
while ((SECONDS < deadline)); do
  if systemctl is-active --quiet "${SERVICE_UNIT}" &&
     ...
     curl -fsS --max-time 5 "${SERVICE_URL}/api/v1/status"; then
    echo
    echo "[restart] calibration service restarted; no capture or device-control request was sent"
