...
class TrayPointExcluder:
    def __init__(self, config: TrayDetectionConfig) -> None:
        ...
        self.sam_secondary_conf_threshold = float(
            np.clip(config.sam_secondary_conf_threshold, 0.0, 1.0)
        )
        ...
        self.hf_cache_dir = prepare_hf_cache_dir(config.hf_cache_dir)
    ...
    def detect(self, frame_bgr: np.ndarray) -> list[TrayDetection]:
        ...
        det_bgr, inv_scale = resize_for_detection(
            frame_bgr=frame_bgr, detect_max_side=self.detect_max_side
        )
        ...
        for idx, (box_det, score, merged_label) in enumerate(filtered_items):
            ...
            mask = self._build_candidate_mask(
                idx, float(score), full_pil, box_full, h, w
            )
            if mask is None:
                continue
        ...
        kept = suppress_masks_by_iou(
            candidates, self.mask_iou_suppress, self.topk_objects
        )
        return [
            TrayDetection(
    ...
    def _detect_prompt_boxes(
        ...
        if self.combine_prompts_forward:
            ...
            return [
                (b, s, merge_label_text(raw_label=lb, prompt_term=""))
                for b, s, lb in box_items
            ]
        ...
        for term in self.prompt_terms:
            term_items = self._grounding_detect_topk(
                pil_img=pil_img, prompt_text=term, topk=per_prompt_topk
            )
            for box_det, score, label_text in term_items:
                box_items.append(
                    (
                        box_det,
                        float(score),
                        merge_label_text(raw_label=label_text, prompt_term=term),
                    )
                )
    ...
    def _grounding_detect_topk(
        ...
        return [
            (box_arr[int(i)], float(score_arr[int(i)]), label_arr[int(i)]) for i in keep
        ]
        ...
        if self.use_sam:
            if not self.sam_primary_only:
                ...
                use_sam_for_this = (
                    ...
                    or float(score) >= self.sam_secondary_conf_threshold
                )
        if use_sam_for_this:
            return self._sam_segment_box(
                pil_img=pil_img, box_xyxy=box_xyxy, out_h=out_h, out_w=out_w
            )
    ...
    def _sam_segment_box(
        ...
        if mask_u8.shape[0] != out_h or mask_u8.shape[1] != out_w:
            mask_u8 = cv2.resize(
                mask_u8, (out_w, out_h), interpolation=cv2.INTER_NEAREST
            )
        return mask_u8
