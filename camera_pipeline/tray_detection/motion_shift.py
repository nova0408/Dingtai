...
def prepare_tracking_gray(
    image_bgr: np.ndarray, max_side: int
...
def estimate_phase_shift(
    ...
    try:
        (dx_small, dy_small), response = cv2.phaseCorrelate(
            np.asarray(ref_gray, dtype=np.float32),
            np.asarray(cur_gray, dtype=np.float32),
        )
    except cv2.error:
        return PhaseShiftEstimate(dx_px=0.0, dy_px=0.0, response=0.0, valid=False)
    ...
    if response_value < float(min_response):
        return PhaseShiftEstimate(
            dx_px=0.0, dy_px=0.0, response=response_value, valid=False
        )
    ...
    return PhaseShiftEstimate(
        dx_px=dx_px, dy_px=dy_px, response=response_value, valid=True
    )
...
def warp_mask(mask: np.ndarray, dx_px: float, dy_px: float) -> np.ndarray:
    h, w = mask.shape[:2]
    mat = np.asarray(
        [[1.0, 0.0, float(dx_px)], [0.0, 1.0, float(dy_px)]], dtype=np.float32
    )
