...
class OpeningDetectionPipeline:
    ...
    def detect_opening(
        ...
        ----------
        rgb_bgr:
            输入彩色图像，形状 `(H, W, 3)`，dtype 通常为 `uint8`。
        tray_mask:
            托盘掩码，形状 `(H, W)`，非零像素表示当前目标托盘区域。
        hp_gray:
            高反差灰度图，形状 `(H, W)`，来自 `build_high_contrast_domain` 的双边滤波输出。
        hp_edge:
            高反差边缘图，形状 `(H, W)`，当前默认采用 `bilateral + canny`。传 `None`
            时仅退化为使用 `hp_gray`，不会重新走父包聚合导入或额外图像路径。
    ...
    def build_high_contrast_domain(bgr: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        ----------
        bgr:
            输入彩色图像，形状 `(H, W, 3)`，dtype 通常为 `uint8`，通道顺序为 BGR。

        Returns
        -------
        highpass_bgr:
            高反差增强图像，形状 `(H, W, 3)`，dtype `uint8`。
        filtered_gray:
            用于开口检测与区域生长的灰度图，形状 `(H, W)`，dtype `uint8`。
        edge_mask:
            默认边缘图，当前固定采用 `bilateral + canny`，形状 `(H, W)`，dtype `uint8`。

        Notes
        -----
        当前默认方案来自本项目现场对比结论：在开口边缘识别中，`bilateral + canny`
