...
from .protocol import BallPoseDetectionRequest, BallPoseDetectionResponse
...
class BallPoseDetectionRpcServer:
    """球位姿检测 RPC 服务端收发封装。"""
    ...
    def recv_request(self) -> BallPoseDetectionRequest:
        return decode_request(self._socket.recv_multipart())

    def send_response(self, response: BallPoseDetectionResponse) -> None:
        self._socket.send_multipart(encode_response(response))


class BallPoseDetectionRpcClient:
    """球位姿检测 RPC 客户端。"""
    ...
    def call(self, request: BallPoseDetectionRequest) -> BallPoseDetectionResponse:
