...
description: 安全维护 Dingtai 根目录 robot_control 统一机器人控制服务。用于修改 qmlinker、AR5 xCoreSDK、HTTP API、设备网关或只读状态验证；强制禁止 Codex、CI、hook 和无人值守流程执行任何控制 POST、运动测试或真实硬件控制。
---
...
3. 复用 `robot_control/devices/` 内的服务本地设备适配器和远端 `sdk/xcoresdk`，不在 HTTP 层复制 SDK 协议细节；
   RobotControl 运行时不得引用仓库 `src/`；设备适配只依赖服务边界内的 Linux xCoreSDK。
...
10. `robot_control/README.md`、`robot_control/API Reference.md`、`robot_control/openapi.yaml` 和
   ...
   配置、端口、单位或安全边界，都必须在同一批改动中同步更新这四个文件。
...
5. 最终报告必须分别列出：静态验证、契约检查、只读验证、未执行的控制测试和未验证的硬件行为；同步/部署结果由
   `ssh-orin-remote-ops` skill 统一报告。
