...
6. 禁止在 class 中散落大量算法参数（例如多个 `self._xxx = 常量`）。涉及 3 个及以上可调参数时，必须抽取为独立 `dataclass` 配置对象，并在字段说明中写明单位、范围与语义。
7. 对于面向固定环境的实验页或实时预览页，不允许使用 `hasattr` / `getattr` 做版本兼容分支来掩盖缺失 API。必须按已确定的环境依赖和单一路径实现，缺失依赖应直接报错并由环境修复解决。

## 参数配置约束（会话补充）

1. 估计器、检测器、滤波器等算法类的可调参数，默认使用 `@dataclass(frozen=True)` 配置类承载。
2. 配置类应与算法类同层或更高层定义，避免把参数常量散落在方法内部和构造函数中。
3. 算法类构造函数优先接收 `config` 对象；当 `config=None` 时显式回退默认配置实例，不允许隐式硬编码多组散落默认值。
4. 配置类字段必须补齐中文说明，至少包含参数含义、单位和推荐范围。
...
1. 当测试脚本、调试脚本或轻量工具只需要某个子目录下的局部模块时，先检查父包 `__init__.py` 是否会聚合导入无关子模块。
2. 如果父包聚合导入会触发可选重依赖，例如 `torch`、`transformers`、硬件 SDK 或 GUI 运行时，则不要为了图省事继续走 `from pkg.subpkg import ...` 这类入口。
3. 此时应优先修正导入边界：例如拆分聚合导出、改走更窄的独立文件加载、或在测试脚本中构造仅覆盖目标目录的局部包加载器，避免无关依赖被隐式拉起。
4. 除了仓库已明确全局忽略的 `E402` 外，其他情况下都不允许使用 `# noqa: E402`、`type: ignore` 或类似 lint 注释掩盖导入异常、导入顺序异常、导入边界异常或包初始化副作用问题；应优先解决真实的导入链设计问题。
5. 如果已经尝试修复导入问题但仍无法在当前回合内安全解决，不允许继续用 lint 注释、假导入或静默回退伪装为“已修复”；必须保留真实失败状态，并在结果中明确暴露问题位置、失败原因、已尝试手段和剩余阻塞，交由人类决定。
6. 修改后至少做一次导入级验证或 `py_compile` 验证，并在结果说明中明确是否仍存在运行时外部依赖边界。
