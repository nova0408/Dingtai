...
description: 统一 Dingtai 仓库静态检查流程。用于在固定 DingTai conda 环境下执行 ruff 与 pyright，并通过 `.agent/tools/check` 提供通用 CLI 入口与 VSCode 任务入口，保证 IDE 与命令行结果一致。
---
...
1. 必须通过 `.agent/tools/check` 下的脚本执行检查，不直接依赖当前 PATH。
...
powershell -ExecutionPolicy Bypass -File .\.agent\tools\check\run_pyright.ps1 -Target .\test
```
...
3. VSCode 任务入口应调用 `.agent/tools/check` 下脚本，不得回退到旧路径。

## 失败处理

1. 若脚本路径失效，先修复 `.vscode/tasks.json` 与 `.agent/tools/check` 路径一致性。
